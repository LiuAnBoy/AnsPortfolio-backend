...
function ProjectEditForm() {
  ...
  const { getProjectDetail } = useGetProjects();
  const { updateProjectByNumber } = usePatchProject();
  ...
  const handleSave = async (value: ProjectProps) => {
    ...
    const res = await updateProjectByNumber(number as string, value);
    if (!res) {
      sendErrorMsg('更新失敗');
      return;
    }
  ...
  useEffect(() => {
    const getProjectData = async () => {
      const res: ProjectProps = await getProjectDetail(number as string);
      if (!res) {
        sendErrorMsg('獲取資料失敗');
        return;
      } else {
  ...
  return (
    <>
      <div className="mb-4 flex item-center justify-between">
        <span className="text-3xl">編輯作品</span>
        <Space>
          <Button
            type="primary"
            size="large"
